  ...
  for currentLevel = lvl, 1, -1 do
    ...
    while key do
      ...
      if valueData.value then
        local value = valueData.value & padding
        valueData.value = nil

        for _, binaryRule in _ipairs(levelData[currentLevel]) do
          local leftValue = binarySearchFunc.left(binaryRule, value)
          local rightValue = binarySearchFunc.right(binaryRule, value + maxOffset)
          ...
          if leftValue and rightValue then
            ...
            for index = leftValue, rightValue do
              local indexAddress = binaryRule[index].address
              local indexValue = binaryRule[index].value
